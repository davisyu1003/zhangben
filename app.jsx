...
const callAI = async (text, imageBase64 = null) => {
  const content = imageBase64
    ? [{ type:"image_url", image_url:{ url:`data:image/jpeg;base64,${imageBase64}` } }, { type:"text", text }]
    : text;
  const resp = await fetch("https://openrouter.ai/api/v1/chat/completions", {
    ...
    body: JSON.stringify({ model: OPENROUTER_MODEL, messages: [{ role:"user", content }], max_tokens: 1000 }),
  });
...
function App() {
  ...
  const handleAiText = async () => {
    ...
    try { const t = await callAI(`从以下描述提取消费记录，返回JSON数组，每项: date(YYYY-MM-DD，无则${todayStr()}),amount(数字),category(餐饮/购物/交通/住房/娱乐/医疗/教育/通讯/人情/其他),note(简短)。只返回JSON数组。描述：${aiInput}`); addRecords(parseJSON(t)); setAiInput(""); setAddMode(null); }
  ...
  const handleImgParse = async () => {
    ...
    try { const t = await callAI(`识别图片中的消费信息，返回JSON数组，每项: date(YYYY-MM-DD，无则${todayStr()}),amount(数字人民币),category(餐饮/购物/交通/住房/娱乐/医疗/教育/通讯/人情/其他),note(商家或商品名)。只返回JSON数组。`, imgBase64); addRecords(parseJSON(t)); setImgPreview(null); setImgBase64(null); setAddMode(null); }
  ...
  const applyFixedNow = item => {
    ...
    addRecords([{date:today,amount:item.amount,category:item.category,note:item.name,fixedId:item.id,isFixed:true}]); setDetailFixed(null);
  };
  ...
  const CatPicker = ({value, onChange}) =>
    React.createElement("div",{style:{display:"flex",flexWrap:"wrap",gap:6}},
      CATS.map(c=>React.createElement("div",{key:c.name,onClick:()=>onChange(c.name),style:{display:"flex",alignItems:"center",gap:4,padding:"6px 12px",borderRadius:20,border:`1.5px solid ${value===c.name?c.color:"#eee"}`,background:value===c.name?c.color+"18":"transparent",cursor:"pointer"}},
        React.createElement("span",{style:{fontSize:14}},c.icon),
        React.createElement("span",{style:{fontSize:13,color:value===c.name?c.color:"#888"}},c.name)
      ))
    );
  ...
  return React.createElement("div",{style:{maxWidth:480,margin:"0 auto",background:"#f7f8fa",minHeight:"100vh",fontFamily:"-apple-system,BlinkMacSystemFont,'PingFang SC',sans-serif"}},
    ...
    tab==="home"&&React.createElement(React.Fragment,null,
      ...
      React.createElement("div",{style:{padding:"12px 16px 100px"}},
        ...
        groupByDate.map(([date,recs])=>React.createElement("div",{key:date,style:{marginBottom:12}},
          ...
          React.createElement("div",{style:{background:"#fff",borderRadius:14,overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,0.05)"}},
            recs.map((r,i)=>React.createElement(RecordRow,{key:r.id,r,i}))
          )
        ))
      )
    ),
    ...
    showAddMenu&&React.createElement("div",{onClick:()=>setShowAddMenu(false),style:{position:"fixed",inset:0,zIndex:98}},
      React.createElement("div",{onClick:e=>e.stopPropagation(),style:{position:"fixed",bottom:80,left:"50%",transform:"translateX(-50%)",display:"flex",gap:14,zIndex:99}},
        [["📷","拍照","image"],["T","文字","text"],["✎","单笔","single"],["🔁","固定","fixed"]].map(([icon,label,mode])=>
          React.createElement("div",{key:label,onClick:()=>{setShowAddMenu(false);setEditFixed(null);resetFixedForm();if(mode==="single")resetForm();setAddMode(mode);},style:{display:"flex",flexDirection:"column",alignItems:"center",gap:6,cursor:"pointer"}},
            React.createElement("div",{style:{width:50,height:50,borderRadius:"50%",background:"#222",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 14px rgba(0,0,0,0.28)"}},React.createElement("span",{style:{color:"#fff",fontSize:19,fontWeight:600}},icon)),
    ...
    // 单笔录入
    addMode==="single"&&React.createElement("div",{style:mBase,onClick:()=>setAddMode(null)},
      React.createElement("div",{style:sh,onClick:e=>e.stopPropagation()},
        React.createElement("div",{style:{fontSize:16,fontWeight:700,marginBottom:16}},"✎ 单笔记账"),
        React.createElement("div",{style:{display:"flex",flexDirection:"column",gap:12}},
          React.createElement("div",null,React.createElement("div",{style:{fontSize:12,color:"#aaa",marginBottom:4}},"日期"),React.createElement("input",{type:"date",value:form.date,onChange:e=>setForm(f=>({...f,date:e.target.value})),style:inp})),
          React.createElement("div",null,React.createElement("div",{style:{fontSize:12,color:"#aaa",marginBottom:4}},"金额（元）"),React.createElement("input",{type:"number",value:form.amount,onChange:e=>setForm(f=>({...f,amount:e.target.value})),placeholder:"0.00",style:inp})),
          React.createElement("div",null,React.createElement("div",{style:{fontSize:12,color:"#aaa",marginBottom:8}},"分类"),React.createElement(CatPicker,{value:form.category,onChange:v=>setForm(f=>({...f,category:v}))})),
          React.createElement("div",null,React.createElement("div",{style:{fontSize:12,color:"#aaa",marginBottom:4}},"备注（可选）"),React.createElement("input",{value:form.note,onChange:e=>setForm(f=>({...f,note:e.target.value})),placeholder:"如：麦当劳午餐",style:inp})),
          React.createElement("button",{onClick:()=>{ if(!form.amount||+form.amount<=0){showToast("请输入有效金额","error");return;} addRecords([{...form}]); resetForm(); setAddMode(null); },style:gBtn},"保存")
        )
      )
    ),
      ...
      React.createElement("div",{style:sh,onClick:e=>e.stopPropagation()},
        ...
        React.createElement("div",{style:{display:"flex",flexDirection:"column",gap:12}},
          React.createElement("div",null,
            React.createElement("div",{style:{fontSize:12,color:"#aaa",marginBottom:4}},"日期"),
            React.createElement("input",{type:"date",value:form.date,onChange:e=>setForm(f=>({...f,date:e.target.value})),style:inp})
          ),
          React.createElement("div",null,
            React.createElement("div",{style:{fontSize:12,color:"#aaa",marginBottom:4}},"金额（元）"),
            React.createElement("input",{type:"number",value:form.amount,onChange:e=>setForm(f=>({...f,amount:e.target.value})),placeholder:"0.00",style:inp})
          ),
          React.createElement("div",null,
            React.createElement("div",{style:{fontSize:12,color:"#aaa",marginBottom:8}},"分类"),
            React.createElement("div",{style:{display:"flex",flexWrap:"wrap",gap:6}},
              CATS.map(c=>React.createElement("div",{key:c.name,onClick:()=>setForm(f=>({...f,category:c.name})),style:{display:"flex",alignItems:"center",gap:4,padding:"6px 12px",borderRadius:20,border:`1.5px solid ${form.category===c.name?c.color:"#eee"}`,background:form.category===c.name?c.color+"18":"transparent",cursor:"pointer"}},
                React.createElement("span",{style:{fontSize:14}},c.icon),
                React.createElement("span",{style:{fontSize:13,color:form.category===c.name?c.color:"#888"}},c.name)
              ))
            )
          ),
          React.createElement("div",null,
            React.createElement("div",{style:{fontSize:12,color:"#aaa",marginBottom:4}},"备注（可选）"),
            React.createElement("input",{value:form.note,onChange:e=>setForm(f=>({...f,note:e.target.value})),placeholder:"如：麦当劳午餐",style:inp})
          ),
          React.createElement("button",{onClick:()=>{
            if(!form.amount||+form.amount<=0){showToast("请输入有效金额","error");return;}
            addRecords([{...form}]); resetForm(); setAddMode(null);
          },style:gBtn},"保存")
        )
      )
    ),
      ...
      React.createElement("div",{style:sh,onClick:e=>e.stopPropagation()},
        ...
        React.createElement("div",{style:{display:"flex",flexDirection:"column",gap:10}},
          ...
          React.createElement("div",null,
            ...
            React.createElement(CatPicker,{value:editForm.category,onChange:v=>setEditForm(f=>({...f,category:v}))})
          ),
